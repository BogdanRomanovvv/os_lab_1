/* child2.c - аналогично child1.c, пишет в child2.txt */
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (write_all(1, line, (size_t)rl) < 0)
            eprint("child2: write stdout failed\n");
        if (fd >= 0)
        {
            if (write_all(fd, line, (size_t)rl) < 0)
                eprint("child2: write file failed\n");
        }
        free(line);
    }
